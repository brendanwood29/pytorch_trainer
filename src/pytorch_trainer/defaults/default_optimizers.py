...
class OptimGetter(AbstractGetter[Type[Optimizer], Optimizer]):
    def __init__(self, optims: Dict[str, Type[Optimizer]] | None = None) -> None:
        """Default optimizer getting object. Pass any optimizer through `optims` as
        a dictionary of strings mapping to Type[Optimizer].
        `AdamW` and `Adam` are preconfigured.

        Args:
            optims (Dict[str, Type[nn.Module]] | None, optional):
                Dictonary mapping optimizers to their names. Defaults to None.
        """
        super().__init__(optims)

        self.registry.update({"adamw": AdamW, "adam": Adam})
